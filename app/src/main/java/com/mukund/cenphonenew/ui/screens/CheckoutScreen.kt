...
fun CheckoutScreen(
    ...
    var name by remember { mutableStateOf(TextFieldValue(customer?.firstname?.plus(" ")?.plus(customer.lastname) ?: "")) }
            ...
            Button(
                onClick = {
                    try {
                        ...
                        android.util.Log.e("CheckoutScreen", "Error proceeding to payment: ${e.message}", e)
                    } finally {
                        isLoading = false
                    }
                },
                modifier = Modifier.fillMaxWidth(),
                enabled = isFormValid && cartItems.isNotEmpty() && !isLoading
            ) {
                if (isLoading) {
